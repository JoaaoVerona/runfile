...
| `runfile.lspPath` | `run` | Path to `run`, asked to serve the language server as `run :lsp`. |
...
The `run` CLI on your `PATH`. That is all — the language server is `run :lsp`, so there is no second binary.
